...
if ( ! function_exists( 'pixelgrade_get_blog_grid_alignment_class' ) ) {
	...
	function pixelgrade_get_blog_grid_alignment_class( $location = '' ) {
		...
		if ( $title_position == 'overlay' ) {
			$title_classes[] = 'c-gallery--title-' . sanitize_html_class( pixelgrade_option( 'blog_items_title_alignment_overlay', 'bottom-left' ) );
		} else {
			$title_classes[] = 'c-gallery--title-' . sanitize_html_class( pixelgrade_option( 'blog_items_title_alignment_nearby', 'left' ) );
		}
			...
			switch ( $meta_key ) {
				case 'category':
					...
					if ( is_page() ) {
						...
						if ( ! empty( $main_category ) ) {
							$category .= '<span class="screen-reader-text">' . esc_html__( 'Main Category', 'noto' ) . "</span><ul>\n";
						...
						if ( ! is_wp_error( $categories ) && ! empty( $categories ) ) {
							$category .= '<span class="screen-reader-text">' . esc_html__( 'Categories', 'noto' ) . "</span><ul>\n";
				...
				case 'tags':
					...
					if ( ! is_wp_error( $post_tags ) && ! empty( $post_tags ) ) {
						$tags .= '<span class="screen-reader-text">' . esc_html__( 'Tags', 'noto' ) . "</span><ul>\n";
				...
				case 'comments':
					if ( comments_open() ) {
						...
						if ( $comments_number == 0 ) {
							$comments = esc_html__( 'No Comments', 'noto' );
						} else {
							/* translators: %d: the number of comments */
							$comments = sprintf( _n( '%d Comment', '%d Comments', $comments_number, 'noto' ), $comments_number );
						}
	...
	function pixelgrade_get_the_post_navigation( $args = array() ) {
		$args = wp_parse_args(
			$args, array(
				...
				'excluded_terms'     => '',
				'taxonomy'           => 'category',
				'screen_reader_text' => esc_html__( 'Post navigation', 'noto' ),
			)
		);
		...
		$previous = get_previous_post_link(
			'<div class="nav-previous"><span class="nav-links__label  nav-links__label--previous">' . esc_html__( 'Previous article', 'noto' ) . '</span><span class="nav-title  nav-title--previous">%link</span></div>',
			$args['prev_text'],
			$args['in_same_term'],
			$args['excluded_terms'],
			$args['taxonomy']
		...
		$next = get_next_post_link(
			'<div class="nav-next"><span class="nav-links__label  nav-links__label--next">' . esc_html__( 'Next article', 'noto' ) . '</span><span class="nav-title  nav-title--next">%link</span></div>',
			$args['next_text'],
			$args['in_same_term'],
			$args['excluded_terms'],
			$args['taxonomy']
...
if ( ! function_exists( 'pixelgrade_get_author_bio_links' ) ) {
	...
	function pixelgrade_get_author_bio_links( $post_id = null ) {
		...
		$markup .= '<a class="c-author__social-link  c-author__website-link h5" href="' . esc_url( $user_posts ) . '" rel="author" title="' . esc_attr( sprintf( esc_html__( 'View all posts by %s', 'noto' ), get_the_author() ) ) . '">' . esc_html__( 'All posts', 'noto' ) . '</a>';
		...
		if ( ! empty( $user_website ) ) {
			$markup .= '<a class="c-author__social-link h5" href="' . esc_url( $user_website ) . '" target="_blank">' . esc_html__( 'Website', 'noto' ) . "</a>\n";
		}
		$markup .= "</span>\n";
...
if ( ! function_exists( 'pixelgrade_entry_footer' ) ) {
	...
	function pixelgrade_entry_footer( $post_id = null ) {
		...
		if ( ! is_single( $post_id ) && ! post_password_required( $post_id ) && ( comments_open( $post_id ) || get_comments_number( $post_id ) ) ) {
			...
			comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'noto' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title( $post_id ) ) );
			echo '</span>';
		}
	...
	function pixelgrade_shape_comment( $comment, $args, $depth ) {
		switch ( $comment->comment_type ) :
			...
			case 'trackback':
				...
				<p><?php esc_html_e( 'Pingback:', 'noto' ); ?><?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'noto' ), ' ' ); ?></p>
				<?php
				break;
			default:
				?>
					...
					<div class="comment__body">
						<header class="c-meta">
							<div class="comment__author vcard">
								...
								printf( wp_kses_post( __( '%s <span class="says">says:</span>', 'noto' ) ), sprintf( '<b class="fn">%s</b>', get_comment_author_link( $comment ) ) );
								?>
							</div><!-- .comment-author -->
							...
							<div class="comment__metadata">
								<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
									<time datetime="<?php esc_attr( get_comment_time( 'c' ) ); ?>">
										...
										printf( esc_html__( '%1$s at %2$s', 'noto' ), esc_html( get_comment_date( '', $comment ) ), esc_html( get_comment_time() ) );
										?>
									</time>
								</a>
								<?php edit_comment_link( esc_html__( 'Edit', 'noto' ), '<span class="edit-link">', '</span>' ); ?>
							</div><!-- .comment-metadata -->

							<?php if ( '0' == $comment->comment_approved ) : ?>
								<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'noto' ); ?></p>
							<?php endif; ?>
						</header><!-- .comment-meta -->
...
if ( ! function_exists( 'pixelgrade_the_post_custom_css' ) ) {
	...
	function pixelgrade_the_post_custom_css( $location = '' ) {
		...
		if ( true === apply_filters( 'pixelgrade_display_the_post_custom_css', true, get_the_ID(), $location ) ) {
			...
			if ( ! empty( $custom_css ) ) {
				$output .= '<div class="custom-css" data-css="' . esc_attr( $custom_css ) . "\"></div>\n";
			}
	...
	function pixelgrade_get_the_posts_pagination( $args = array() ) {
		...
		$args = wp_parse_args(
			$args, array(
				...
				'mid_size'           => 2,
				'type'               => 'list',
				'prev_text'          => esc_html_x( '&laquo; Previous', 'previous set of posts', 'noto' ),
				'next_text'          => esc_html_x( 'Next &raquo;', 'next set of posts', 'noto' ),
				'screen_reader_text' => esc_html__( 'Posts navigation', 'noto' ),
			)
		);
...
if ( ! function_exists( 'pixelgrade_posted_on' ) ) {
	...
	function pixelgrade_posted_on() {
		...
		printf( '<span class="byline"> <span class="by">%1$s</span> <span class="author vcard"><a class="url fn n" href="%2$s">%3$s</a></span></span><span class="posted-on"><a href="%4$s" rel="bookmark">%5$s</a></span>',
			esc_html_x( 'by', 'post author', 'noto' ),
