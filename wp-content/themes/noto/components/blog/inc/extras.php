...
function pixelgrade_add_cats_list( $content ) {
	...
	if ( apply_filters( 'pixelgrade_add_categories_to_content', $add ) ) {
		...
		if ( ! empty( $categories_list ) && 'Uncategorized' !== $categories_list ) {
			$cats_content .= '<div class="cats"><span class="cats__title">' . esc_html__( 'Categories', 'noto' ) . '</span>' . $categories_list . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput
		}
	}
...
function pixelgrade_add_tags_list( $content ) {
	...
	if ( apply_filters( 'pixelgrade_add_tags_to_content', $add ) ) {
		...
		if ( ! empty( $tags_list ) ) {
			$tags_content .= '<div class="tags"><div class="tags__title">' . esc_html__( 'Tags', 'noto' ) . '</div>' . $tags_list . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput
		}
	}
...
if ( ! function_exists( 'pixelgrade_search_form' ) ) :
	...
	function pixelgrade_search_form( $form ) {
		...
		<label class="screen-reader-text">' . esc_html__( 'Search for:', 'noto' ) . '</label>
		<input type="text" placeholder="' . esc_attr__( 'Search here', 'noto' ) . '" value="' . esc_attr( get_search_query() ) . '" name="s" class="search-field" />
		<button type="submit" class="search-submit"><span>' . esc_html__( 'Search', 'noto' ) . '</span></button>
		</form>';
...
if ( ! function_exists( 'pixelgrade_is_page_for_projects' ) ) {
	...
	function pixelgrade_is_page_for_projects( $page_id = null ) {
		...
		if ( ! isset( $wp_query ) ) {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Conditional query tags do not work before the query is run. Before then, they always return false.', 'noto' ), '3.1.0' );
