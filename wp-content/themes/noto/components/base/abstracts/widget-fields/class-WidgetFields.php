...
if ( ! class_exists( 'Pixelgrade_WidgetFields' ) ) :
	...
	abstract class Pixelgrade_WidgetFields extends WP_Widget {
		...
		public function enqueueAdminFieldsScripts() {
			...
			wp_localize_script(
				'pixelgrade-widget-fields-js', 'pixelgradeWidgetFields', array(
					'image' => array(
						'frame_title'  => esc_html__( 'Select an Image', 'noto' ),
						'button_title' => esc_html__( 'Insert Into Widget', 'noto' ),
					),
				)
			);
				...
				foreach ( $sections as $section_id => $section ) {
					...
					if ( ! empty( $section_fields ) ) {
						if ( $do_accordion ) {
							...
							echo '<h3 class="accordion-section-title hndle">' . esc_html( ! empty( $section['title'] ) ? $section['title'] : '' ) . '<span class="screen-reader-text">' . esc_html__( 'Press return or enter to open this section.', 'noto' ) . "</span></h3>\n";
		...
		public function displayField_image( $field_name, $field_config, $instance ) {
			...
			$button_label = esc_html__( 'Select Image', 'noto' );
			if ( ! empty( $field_config['button_label'] ) ) {
				$button_label = $field_config['button_label'];
			}

			$clear_label = esc_html__( 'Clear', 'noto' );
		...
		public function sidebarNotSupportedMessage( $args, $instance ) {
			...
			$html_message = '<div class="c-alert  c-alert--danger">
                    <h4 class="c-alert__title">'. esc_html__( '🤦 Widget Type Not Supported Here', 'noto' ) . '</h4>
                    <div class="c-alert__body">
                        <p>' .
			                /* translators: %s: the widget name */
			                sprintf( esc_html__( 'Oops! The %s is not supported in this area, but don\'t panic . You can try to move it to another section or just replace it . ', 'noto' ), '<em>' . esc_html( $args['widget_name'] ) . '</em>' ) .'</p>
                    </div>
                </div>';
