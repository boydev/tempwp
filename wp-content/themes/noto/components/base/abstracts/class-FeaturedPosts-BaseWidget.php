...
if ( ! class_exists( 'Pixelgrade_FeaturedPosts_BaseWidget' ) ) :
	...
	abstract class Pixelgrade_FeaturedPosts_BaseWidget extends Pixelgrade_WidgetFields {
		...
		public function __construct( $id = 'pixelgrade-featured-posts', $name = '', $widget_ops = array(), $config = array() ) {
			...
			$default_config = array(
				'fields_sections'        => array(
					...
					'content' => array(
						'title'         => esc_html__( 'Content', 'noto' ),
						'default_state' => 'open',
						'priority'      => 10,
					),
					'layout'  => array(
						'title'    => esc_html__( 'Layout', 'noto' ),
						'priority' => 20,
					),
					'display' => array(
						'title'    => esc_html__( 'Display', 'noto' ),
						'priority' => 30,
					),
					'others'  => array(
						'title'    => esc_html__( 'Others', 'noto' ),
						'priority' => 40,
					),
				),
				'fields'                 => array(
					...
					'title'                   => array(
						'type'     => 'text',
						'label'    => esc_html__( 'Section Title:', 'noto' ),
						'default'  => esc_html__( 'My Featured Posts', 'noto' ),
						'section'  => 'default',
						'priority' => 10,
					),
					...
					'source'                  => array(
						'type'     => 'radio_group',
						'label'    => esc_html__( 'Posts Source:', 'noto' ),
						'options'  => array(
							'recent'   => esc_html__( 'Recent Posts', 'noto' ),
						),
						/* translators: %s: the theme name */
						'desc'     => sprintf( esc_html__( '👉 More Posts Source options (e.g. Category, Tag, Selected Posts) are available in the Pro version of %s. C\'mon, aim for more, mate!', 'noto' ), pixelgrade_get_original_theme_name() ),
						'default'  => 'recent',
						'section'  => 'content',
						'priority' => 10,
					),
					'source_category'         => array(
						'type'              => 'select',
						'label'             => esc_html__( 'Category:', 'noto' ),
					...
					'post_ids' => array(
						'disabled' => true,
						'type'       => 'text',
						'label'      => esc_html__( 'Post IDs:', 'noto' ),
						'desc'       => esc_html__( 'Use Posts IDs, separated by commas, to show only a set of specific posts.', 'noto' ),
						'default'    => '',
						'display_on' => array(
							'display' => true,
							'on'      => array(
					...
					'orderby' => array(
						'disabled' => true,
						'type'       => 'select',
						'label'      => esc_html__( 'Order by:', 'noto' ),
						'options'    => array(
							'date'    => esc_html__( 'Date', 'noto' ),
							'popular' => esc_html__( 'Most Popular', 'noto' ),
						),
						'default'    => 'date',
						'display_on' => array(
							'display' => false,
					...
					'number'                  => array(
						'type'              => 'number',
						'label'             => esc_html__( 'Number of posts:', 'noto' ),
					...
					'prevent_duplicate_posts' => array(
						'type'     => 'checkbox',
						'label'    => esc_html__( 'Prevent Duplicate Posts', 'noto' ),
						'desc'     => esc_html__( 'The posts displayed by this widget won\'t show up in the next widgets.', 'noto' ),
						'default'  => true,
						'section'  => 'content',
						'priority' => 70,
					),
					...
					'columns'                 => array(
						'type'     => 'select',
						'label'    => esc_html__( 'Number of columns:', 'noto' ),
						'options'  => array(
							'1' => esc_html__( '1 Column', 'noto' ),
							'2' => esc_html__( '2 Columns', 'noto' ),
							'3' => esc_html__( '3 Columns', 'noto' ),
							'4' => esc_html__( '4 Columns', 'noto' ),
						),
						'default'  => '3',
						'section'  => 'layout',
						'priority' => 10,
					...
					'image_ratio'             => array(
						'type'     => 'select',
						'label'    => esc_html__( 'Image Aspect Ratio:', 'noto' ),
						'options'  => array(
							'portrait'  => esc_html__( 'Portrait', 'noto' ),
							'square'    => esc_html__( 'Square', 'noto' ),
							'landscape' => esc_html__( 'Landscape', 'noto' ),
						),
						'default'  => 'portrait',
						'section'  => 'layout',
						'priority' => 20,
					...
					'show_excerpt' => array(
						'disabled' => false,
						'type'     => 'checkbox',
						'label'    => esc_html__( 'Show Excerpt', 'noto' ),
						'default'  => true,
						'section'  => 'display',
						'priority' => 10,
					),
					...
					'show_readmore' => array(
						'disabled' => false,
						'type'     => 'checkbox',
						'label'    => esc_html__( 'Show "Read More" Link', 'noto' ),
						'default'  => true,
						'section'  => 'display',
						'priority' => 20,
					),
					'primary_meta'            => array(
						'type'     => 'select',
						'label'    => esc_html__( 'Primary Meta:', 'noto' ),
						'options'  => array(
					...
					'secondary_meta'          => array(
						'type'     => 'select',
						'label'    => esc_html__( 'Secondary Meta:', 'noto' ),
						/* translators: %s: the theme name */
						'desc'     => sprintf( esc_html__( '👉 More display options (e.g. Show Excerpt, Show Read More) are available in the Pro version of %s. Don\'t let us stop you!', 'noto' ), wp_get_theme( get_template() )->get('Name') ),
						'options'  => array(
							'none'     => esc_html__( 'None', 'noto' ),
							'date'     => esc_html__( 'Date', 'noto' ),
						),
					...
					'show_view_more' => array(
						'disabled' => true,
						'type'       => 'checkbox',
						'label'      => esc_html__( 'Show View More Button', 'noto' ),
						'default'    => false,
						'display_on' => array(
							'display' => false,
							'on'      => array(
					...
					'view_more_label'         => array(
						'type'       => 'text',
						'label'      => esc_html__( 'Label:', 'noto' ),
						'default'    => esc_html__( 'View More', 'noto' ),
						'display_on' => array(
							'display' => true,
							'on'      => array(
			...
			if ( pixelgrade_user_has_access( 'pro-features' ) ) {
				...
				$default_config['fields']['source']['options'] = array(
					'recent'   => esc_html__( 'Recent Posts', 'noto' ),
					'category' => esc_html__( 'Category', 'noto' ),
					'tag'      => esc_html__( 'Tag', 'noto' ),
					'post_ids' => esc_html__( 'Selected Posts', 'noto' ),
				);
			...
			$widget_ops = wp_parse_args(
				$widget_ops, array(
					...
					'description'                 => esc_html__( 'Your featured posts.', 'noto' ),
			...
			if ( empty( $name ) ) {
				$name = esc_html__( 'Featured Posts', 'noto' );
			}
		...
		public function categoriesDropdown( $selected, $field_name, $field_config ) {
			...
			$args         = array(
				'show_option_all'   => esc_html__( 'All', 'noto' ),
				'orderby'           => 'id',
				'order'             => 'ASC',
				'show_count'        => 1,
				'hide_empty'        => 1,
			...
			if ( empty( $cat_dropdown ) ) {
				$output .= '<br /><small>' . esc_html__( 'Please define some categories first.', 'noto' ) . "</small>\n";
			} else {
				$output .= $cat_dropdown;
			}
		...
		public function tagsDropdown( $selected, $field_name, $field_config ) {
			...
			if ( empty( $tag_dropdown ) ) {
				$output .= '<br /><small>' . esc_html__( 'Please define some tags first.', 'noto' ) . "</small>\n";
			} else {
				$output .= $tag_dropdown;
			}
