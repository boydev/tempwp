					...
					if ( ! empty( $this->loop_template_part ) ) {
						...
						if ( is_array( $this->loop_template_part ) ) {
							if ( empty( $this->loop_template_part['slug'] ) ) {
								...
								_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You haven\'t provided a slug for the loop template part for the %s page template custom loop.', 'noto' ), '<code>' . $this->page_slug . '</code>' ), null ); // phpcs:ignore
							} else {
						...
						if ( ! file_exists( $loop_template ) ) {
							...
							_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( '%1$s does not exist. Check out the config of the %2$s custom page template.', 'noto' ), '<code>' . $loop_template_part_slug . '-' . $loop_template_part_name . '</code>', '<code>' . $this->page_slug . '</code>' ), null ); // phpcs:ignore
						} else {
							// These will be available in the scope of the loop template part to do stuff like the_post() and so on.
							$post_template_part_slug = '';
							$post_template_part_name = '';
							if ( is_array( $this->post_template_part ) ) {
								if ( empty( $this->post_template_part['slug'] ) ) {
									/* translators: %s: the page slug */
									_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You haven\'t provided a slug for the post template part for the %s page template custom loop.', 'noto' ), '<code>' . $this->page_slug . '</code>' ), null ); // phpcs:ignore
								} else {
							...
							include $loop_template; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
						}
					} else {
