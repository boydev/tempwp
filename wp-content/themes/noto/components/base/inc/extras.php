...
function pixelgrade_autoload_dir( $path, $depth = 0, $method = 'require_once' ) {
	...
	if ( strpos( $path, wp_normalize_path( ABSPATH ) ) === 0 && strpos( $path, wp_normalize_path( get_template_directory() ) ) !== 0 ) {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Please provide only paths in the theme for autoloading.', 'noto' ), null );

		return false;
	}

	if ( ! in_array( $method, array( 'require', 'require_once', 'include', 'include_once' ) ) ) {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'We support only require, require_once, include, and include_once.', 'noto' ), null );
...
if ( ! function_exists( 'pixelgrade_get_post_details' ) ) {
	...
	function pixelgrade_get_post_details() {
		...
		$details['read_more'] = '<a href="' . esc_url( get_the_permalink() ) . '" class="c-card__action">' . esc_html__( 'Read More', 'noto' ) . '</a>';
		$details['none']      = null;
