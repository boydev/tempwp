		...
		if ( in_array( $hook, array( 'post.php', 'post-new.php', 'page-new.php', 'page.php' ) ) ) {
			...
			wp_localize_script(
				'pixelgrade_hero-admin-metaboxes-scripts', 'pixelgrade_hero_admin', array(
					...
					'featured_projects_ids_helper' => esc_html__( 'Here are the IDs of the selected featured projects to use elsewhere, if the need arises: ', 'noto' ),
				)
			);
		}
	}
	...
	public function metaboxesConfig( $metaboxes ) {
		...
		$hero_metaboxes = array(
			...
			'hero_area_background__page' => array(
				...
				'title'                  => esc_html__( 'Hero Area &#187; Background', 'noto' ),
				...
				'fields'                 => array(
					array(
						'name' => esc_html__( 'Gallery Image', 'noto' ),
						'id'   => '_hero_background_gallery',
						'type' => 'gallery',
					),
					array(
						'name' => esc_html__( 'Playlist', 'noto' ),
						'id'   => '_hero_background_videos',
						'type' => 'playlist',
					),
					array(
						'name'      => esc_html__( 'Image Opacity', 'noto' ),
						'desc'      => '<strong>' . esc_html__( 'Image Opacity', 'noto' ) . '</strong>',
					...
					array(
						'name' => esc_html__( 'Background Color', 'noto' ),
						'desc' => '<strong>' . esc_html__( 'Background Color', 'noto' ) . '</strong> <span class="tooltip" title="<p>' . esc_html__( 'Used as a background color during page transitions.', 'noto' ) . '</p><p>' . esc_html__( 'Tip: It helps if the color matches the background color of the Hero image.', 'noto' ) . '</p>"></span>',
			...
			'hero_area_content__page'    => array(
				...
				'title'                  => '&#x1f535; ' . esc_html__( 'Hero Area &#187; Content', 'noto' )
	                            // @todo We should do something about using markup in an attribute.
								. ' <span class="tooltip" title="<' . 'title>'
								. esc_html__( 'Hero Area &#187; Content', 'noto' )
								. '</title><p>'
								. wp_kses( __( 'Use this section to add a <strong>Title</strong> or a summary for this page. Get creative and add different elements like buttons, logos or other headings.', 'noto' ), wp_kses_allowed_html() )
								. '</p><p>'
								. wp_kses( __( 'You can insert a title using a <strong>Heading 1</strong> element, either on the Hero Area or using a <b>Text Block</b> within the above content area.', 'noto' ), wp_kses_allowed_html() )
								. '</p><p>'
								. wp_kses( __( '* Note that the <strong>Page Title</strong> written above will <u>not</u> be included automatically on the page, so you have complete freedom in choosing where you place or how it looks.', 'noto' ), wp_kses_allowed_html() )
								. "</p><p><a href='#'>"
								. esc_html__( 'Learn more about Managing the Hero Area', 'noto' )
								. '</a></p>"></span>',
				'pages'                  => array( 'page' ), // Post type
				'context'                => 'normal',
				'priority'               => 'high',
				...
				'fields'                 => array(
					array(
						'name'       => esc_html__( 'Description', 'noto' ),
						...
						'desc'       => '<span class="hero-editor-visibility-status">
								...
								<span class="hero-visibility-text">' . esc_html__( 'Visible Hero Area', 'noto' ) . '</span>
								<span class="hero-hidden-text">' . esc_html__( 'Hidden Hero Area', 'noto' ) . '</span>
								</span>
								<span class="hero-visibility-description">' . esc_html__( 'To hide the Hero Area section, remove the content above and any item from the Hero Area &#187; Background.', 'noto' ) . '</span>
								<span class="hero-hidden-description">' . esc_html__( 'Add some content above or an image to the Hero Area &#187; Background to make the Hero Area visible.', 'noto' ) . '</span>',
					...
					array(
						'name'    => esc_html__( 'Hero Area Height', 'noto' ),
						'desc'    => '<p>' . esc_html__( 'Set the height of the Hero Area relative to the browser window.', 'noto' ) . '</p>',
						'id'      => '_hero_height',
						'type'    => 'select',
						'options' => array(
							array(
								'name'  => '&#9673;&#9673;&#9673; ' . esc_html__( 'Full Height', 'noto' ),
								'value' => 'c-hero--full',
							),
							array(
								'name'  => '&#9673;&#9673;&#9711; ' . esc_html__( 'Two Thirds', 'noto' ),
								'value' => 'c-hero--two-thirds',
							),
							array(
								'name'  => '&nbsp; &#9673;&#9711; ' . esc_html__( '&nbsp;Half', 'noto' ),
								'value' => 'c-hero--half',
							),
						),
					...
					array(
						'name'    => esc_html__( 'Hero Content Alignment', 'noto' ),
						'desc'    => wp_kses(
							__(
					...
					array(
						'name'  => '&#x1F307; &nbsp; ' . esc_html__( 'Slideshow Options', 'noto' ),
						'id'    => '_hero_slideshow_options__title',
						'value' => wp_kses( __( 'Add more than one image to the <strong>Hero Area &#187; Background</strong> to enable this section. ', 'noto' ), wp_kses_allowed_html() ),
						'type'  => 'title',
					),
					array(
						'name'    => esc_html__( 'Auto Play', 'noto' ),
						'desc'    => esc_html__( 'The slideshow will automatically move to the next slide, after a period of time.', 'noto' ),
						'id'      => '_hero_slideshow_options__autoplay',
						'type'    => 'select',
						'options' => array(
							array(
								'name'  => esc_html__( 'Enabled', 'noto' ),
								'value' => true,
							),
							array(
								'name'  => esc_html__( 'Disabled', 'noto' ),
								'value' => false,
							),
						),
						'std'     => false,
					...
					array(
						'name'       => esc_html__( 'Auto Play Delay (s)', 'noto' ),
						'desc'       => esc_html__( 'Set the number of seconds to wait before moving to the next slide.', 'noto' ),
			...
			'hero_area_map__page'        => array(
				...
				'title'      => esc_html__( 'Map Coordinates & Display Options', 'noto' ),
				'pages'      => array( 'page' ), // Post type
				'context'    => 'normal',
				'priority'   => 'high',
				'hidden'     => true,
				...
				'fields'     => array(
					array(
						'name'    => esc_html__( 'Map Height', 'noto' ),
						'desc'    => '<p>' . esc_html__( 'Select the height of the Google Map area in relation to the browser window.', 'noto' ) . '</p>',
						'id'      => '_hero_map_height',
						'type'    => 'select',
						'options' => array(
							array(
					...
					array(
						'name' => esc_html__( 'Google Maps URL', 'noto' ),
						'desc' => wp_kses( __( 'Paste here the <strong>Share Link</strong>URL you have copied from <a href="https://www.google.com/maps" target="_blank">Google Maps</a>. Do not use the embed code or a short URL.', 'noto' ), wp_kses_allowed_html() ),
						'id'   => '_hero_map_url',
						'type' => 'textarea_small',
						'std'  => '',
					),
					array(
						'name' => esc_html__( 'Custom Colors', 'noto' ),
						'desc' => esc_html__( 'Allow us to change the map colors to better match your website.', 'noto' ),
						'id'   => '_hero_map_custom_style',
						'type' => 'checkbox',
						'std'  => 'on',
					),
					array(
						'name'    => esc_html__( 'Pin Content', 'noto' ),
						'desc'    => esc_html__( 'Insert here the content of the location marker - leave empty for no custom marker.', 'noto' ),
	...
	public function _addVideoUrlFieldToAttachments( $form_fields, $post ) {
		...
		if ( ! isset( $form_fields['link_media_to'] ) ) {
			...
			$select_options = array(
				...
				'media_file'       => esc_html__( 'Media File', 'noto' ),
				'custom_image_url' => esc_html__( 'Custom Image URL', 'noto' ),
				'custom_video_url' => esc_html__( 'Custom Video URL', 'noto' ),
				'external'         => esc_html__( 'External URL', 'noto' ),
			);
			...
			$form_fields['link_media_to'] = array(
				'label' => esc_html__( 'Linked To', 'noto' ),
				'input' => 'html',
				'html'  => $select_html,
			);
		}
		...
		if ( ! isset( $form_fields['video_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
			$form_fields['video_url'] = array(
				'label' => esc_html__( 'Custom Video URL', 'noto' ),
				'input' => 'text', // this is default if "input" is omitted
				'value' => esc_url( get_post_meta( $post->ID, '_video_url', true ) ),
				'helps' => '<p class="desc">' . wp_kses( __( 'Attach a video to this image <span class="small">(YouTube or Vimeo)</span>.', 'noto' ), wp_kses_allowed_html() ) . '</p>',
			);
		}
		...
		if ( ! isset( $form_fields['custom_image_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_image_url' ) {
			$form_fields['custom_image_url'] = array(
				'label' => esc_html__( 'Custom Image URL', 'noto' ),
				'input' => 'text', // this is default if "input" is omitted
				'value' => esc_url( get_post_meta( $post->ID, '_custom_image_url', true ) ),
				'helps' => '<p class="desc">' . esc_html__( 'Link this image to a custom url.', 'noto' ) . '</p>',
			);
		}
		...
		if ( ! isset( $form_fields['video_autoplay'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
			...
			$form_fields['video_autoplay'] = array(
				'label' => esc_html__( 'Video Autoplay', 'noto' ),
				'input' => 'html',
				'html'  => '<input' . $checked . ' type="checkbox" name="attachments[' . esc_attr( $post->ID ) . '][video_autoplay]" id="attachments[' . esc_attr( $post->ID ) . '][video_autoplay]" /><label for="attachments[' . esc_attr( $post->ID ) . '][video_autoplay]">' . esc_html__( 'Enable Video Autoplay?', 'noto' ) . '</label>',
			);
		}

		if ( ! isset( $form_fields['external_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'external' ) {
			$form_fields['external_url'] = array(
				'label' => esc_html__( 'External URL', 'noto' ),
				'input' => 'text',
				'value' => esc_url( get_post_meta( $post->ID, '_external_url', true ) ),
				'helps' => '<p class="desc">' . esc_html__( 'Set this image to link to an external website.', 'noto' ) . '</p>',
			);
		}
